...
static int _thread_update_node_energy(void)
{
	...
	if (!xcc_energy.poll_time) {
		...
		/* Detect an overflow */
		int64_t offset = xcc_raw->j + (IPMI_XCC_OVERFLOW * overflows) -
	...
	/*
	 * FIXME: base_watts is only used in sview?
	 * to print out Lowest Joules?  That seems confusing.  We should remove
	 * it in the future.
	 */

	if (debug_flags & DEBUG_FLAG_ENERGY) {
		info("%s: XCC current_watts: %u consumed energy last interval: %"PRIu64"(current reading %"PRIu64") Joules, elapsed time: %u Seconds, first read energy counter val: %"PRIu64,
		     __func__,
		     xcc_energy.current_watts,
		     xcc_energy.base_consumed_energy,
		     xcc_energy.consumed_energy,
